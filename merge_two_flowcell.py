...
for each_key in dict_a:
    ...
    if each_key not in dict_b:
        line_b = "\t".join(["0" for x in range(len(dict_a[each_key]))])
        output.write(each_key + "\t"+line_a + "\t" + line_b +"\n")

for each_key in dict_b:
    if each_key not in dict_a:
        line_a = "\t".join(["0" for x in range(len(dict_b[each_key]))])
        line_b = "\t".join(dict_b[each_key])
        output.write(each_key + "\t"+line_a + "\t" + line_b +"\n")